...
> [!IMPORTANT]
> 应用默认选择 **Real** 模式，但启动会枚举网卡并自动尝试连接、扫描从站；不会写入硬件。Demo / Mock 仅可在“设置”中启用，并会在界面中明确标识。Real 模式下的状态切换、ESC 寄存器和 EEPROM 写入可能影响机械设备或使从站暂时不可用，请只在隔离、安全的测试环境使用。
...
首次接触真机建议执行：枚举网卡 → 自动连接与扫描 → 读取状态/AL → 读取寄存器 → 读取并离线保存 EEPROM BIN。确认备份可解析且设备处于安全状态后，再在隔离测试从站上验证写入。
